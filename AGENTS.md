...
- **Client auto-configuration**: the plugin configures 23 MCP clients from a registry of
  data-only descriptors; adding one is a new `clients/<name>.gd` plus one script path in
  `_registry.gd::_CLIENT_SCRIPT_PATHS`, with no strategy edits. All descriptors are command-shape (`godot-ai
