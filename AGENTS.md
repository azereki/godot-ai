...
On Windows, the harness logic is platform-agnostic but the *invocation* is POSIX — use `.venv\Scripts\python.exe`, run `serve-this-worktree` under Git Bash/WSL (or start the `--reload` server by hand), and note `$TMPDIR` is `%TEMP%`. See the "Windows / cross-platform notes" callout in `docs/STRESS_TESTING.md` (resilience tracked in #509).
